...
export default function Trains(props) {
    ...
    async function trainTimes(data) {
        const TRAIN_API_URL = `https://can-cors.herokuapp.com/http://lapi.transitchicago.com/api/1.0/ttarrivals.aspx?key=${TRAIN_API_KEY}&stpid=${props.stationNumber || '30012'}&outputType=JSON`;
    ...
    function cleanTime(data) {
        ...
        const diff = Math.abs(new Date('2020/10/09 ' + hourMinutes) - new Date('2011/10/09 ' + currentTime));
        ...
        <div className="predictions trains">
            ...
            ? trains.map(train => (
                ...
                <p className="prediction-route-number">
                    {'To ' + train.destNm}
                    <span className="prediction-route-number-direction">{train.staNm} Station</span>
                    </p>
                    <p className="prediction-time">
                    {cleanTime(train.arrT) < 2 ? "Due" : cleanTime(train.arrT) + ` mins`}
                    </p>
                </div>
            )) : (
